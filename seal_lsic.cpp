...
//vector<uint64_t> seal_lsic(a, mpz_t b,int n,int block,int l)
vector<uint64_t> seal_lsic(Plaintext a, Plaintext b,SEALContext context,SecretKey secret_key,PublicKey public_key,RelinKeys relin_keys,int l)
{
	cout<<"------------------------LSIC------------------------"<<"\n"<<endl;
    ...
    batch_encoder.decode(a, matrix_a);
	
    cout<<"a:"<<endl;
    print_matrix(matrix_a, row_size);
    
    batch_encoder.decode(b, matrix_b);
    ...
    Plaintext plain1_result;
    Plaintext plain_result;

    decryptor.decrypt(t_encrypted, plain1_result);
    vector<uint64_t> pod1_result;
    vector<uint64_t> pod_result;
    batch_encoder.decode(plain1_result, pod1_result);
    cout << "t:" << endl;
    print_matrix(pod1_result, row_size);
